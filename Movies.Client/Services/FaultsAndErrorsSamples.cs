...
public class FaultsAndErrorsSamples : IIntegrationService
{
    ...
    public async Task PostMovieAndHandleErrorsAsync(CancellationToken cancellationToken)
    {
        ...
        var serializedMovieForCreation = JsonSerializer.Serialize(
            movieForCreation, _jsonSerializerOptionsWrapper.Options);
                ...
                request, HttpCompletionOption.ResponseHeadersRead, cancellationToken))
            {
                if (!response.IsSuccessStatusCode)
                {
                    if (response.StatusCode == HttpStatusCode.BadRequest)
                    {
                        ...
                        var errorAsProblemDetails = await
                        JsonSerializer.DeserializeAsync<ExtendedProblemDetailsWithErrors>(
                            errorStream, _jsonSerializerOptionsWrapper.Options);
